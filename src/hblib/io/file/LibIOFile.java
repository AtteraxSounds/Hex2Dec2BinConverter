...
//		}
	...
	/**
	 * Creates path if it does not exist. Does nothing if it exists. See
	 * {@link https://stackoverflow.com/questions/28947250/create-a-directory-if-it-does-not-exist-and-then-create-the-files-in-that-direct}
	 * 
	 * @param sPath
	 */
	public static void createDirectory(String sPath) {
		try {
			...
			System.out.println("**ERROR** @ LibIOFile.createDirectory");
			e.printStackTrace();
		}
	}

	public static void copy(String sPathToFile, String sPathToDirectory) {

	}

	public static void copy(Path pPathToFile, Path pPathToDirectory) {
		try {
			Files.copy(pPathToFile, pPathToDirectory, StandardCopyOption.REPLACE_EXISTING);
		} catch (IOException e) {
			System.out.println("**ERROR** @ LibIOFile.copy");
			e.printStackTrace();
		}
	}
